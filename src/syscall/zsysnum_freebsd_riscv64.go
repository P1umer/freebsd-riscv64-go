...
const (
	...
	SYS_LINK                   = 9   // int link(_In_z_ const char *path,_In_z_ const char *link);
	...
	SYS_PTRACE                 = 26  // int ptrace(int req,pid_t pid,_Inout_opt_ _Contains_long_ptr_ caddr_t addr,int data);
	...
	SYS_IOCTL                  = 54  // int ioctl(int fd,u_long com,_Inout_opt_ _Contains_long_ptr_ char *data);
	...
	SYS_MPROTECT               = 74  // int mprotect(_In_ void *addr,size_t len,int prot);
	...
	SYS_GETGROUPS              = 79  // int getgroups(int gidsetsize,_Out_writes_opt_(gidsetsize) gid_t *gidset);
	SYS_SETGROUPS              = 80  // int setgroups(int gidsetsize,_In_reads_(gidsetsize) const gid_t *gidset);
	...
	SYS_JAIL                   = 338 // int jail(_In_ _Contains_ptr_ struct jail *jail);
	...
	SYS_AIO_WAITCOMPLETE       = 359 // ssize_t aio_waitcomplete(_Outptr_result_maybenull_ struct aiocb **aiocbp,_In_opt_ _Contains_long_timet_ struct timespec *timeout);
	...
	SYS_THR_CREATE             = 430 // int thr_create(_In_ _Contains_long_ptr_ ucontext_t *ctx,_Out_ long *id,int flags);
	...
	SYS_CPUSET_SETDOMAIN       = 562 // int cpuset_setdomain(cpulevel_t level,cpuwhich_t which,id_t id,size_t domainsetsize,_In_ domainset_t *mask,int policy);
	...
	SYS_AIO_WRITEV             = 578 // int aio_writev(_Inout_ _Contains_long_ptr_ struct aiocb *aiocbp);
	SYS_AIO_READV              = 579 // int aio_readv(_Inout_ _Contains_long_ptr_ struct aiocb *aiocbp);
	SYS_FSPACECTL              = 580 // int fspacectl(int fd,int cmd,_In_ const struct spacectl_range *rqsr,int flags,_Out_opt_ struct spacectl_range *rmsr,);
	SYS_SCHED_GETCPU           = 581 // int sched_getcpu(void);
	SYS_SWAPOFF                = 582 // int swapoff(_In_z_ const char *name,u_int flags,);
)
